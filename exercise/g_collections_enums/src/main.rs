...
fn main() {
    ...
    for coord in arrow_coords {
        ...
        let current_shot: Shot = match coord.distance_from_center() {
           v if v < 1.0  => {Shot::Bullseye}, // v becomes the value of the match expression parameter
           v if (v >= 1.0 && v <= 5.0) => {Shot::Hit(v)},
           v if v > 5.0 => {Shot::Miss}
           _ => break
        };
